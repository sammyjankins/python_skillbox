...
def get_ganados(ganados=None):
    ...
    if ganados is None:
        for i in range(1022, 9877):
            number = str(i)
            if len(set(number)) is len(number):
        ...
        if check_sum is 4:
            ...
            for number in bunch_of_numbers:
                if check_sets(number, last_attempt) is 4:
                    new_bunch.append(number)
            bunch_of_numbers = new_bunch
        elif ganados["bulls"] is 0:
            bad_options = set()
            for letter in last_attempt:
                for number in bunch_of_numbers:
                    if last_attempt.find(letter) is number.find(letter):
            ...
            new_bunch = []
            for number in bunch_of_numbers:
                if check_sets(number, last_attempt) is not 0:
                    new_bunch.append(number)
            bunch_of_numbers = new_bunch
